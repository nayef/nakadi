        ...
        try {
            ...
            while (connectionReady.get() &&
                ...
                if (eventOrEmpty.isPresent()) {
                    ...
                    latestOffsets.put(event.getNextPosition().getPartition(), event.getNextPosition());

                    // put message to batch
                    currentBatches.get(event.getNextPosition().getPartition()).add(event.getEvent());
                    messagesRead++;

                    // if we read the message - reset keep alive counter for this partition
                    keepAliveInARow.put(event.getNextPosition().getPartition(), 0);
                }
