...
public class PartitionsController {
    ...
    private List<TopicPartition> loadTopicViews(final EventType eventType) throws ServiceUnavailableException {
        final Map<String, Cursor> newestPostitions =
                topicRepository.loadNewestPosition(eventType.getTopic()).stream().collect(
                        Collectors.toMap(TopicPosition::getPartition, Cursor::fromTopicPosition));
        final Map<String, Cursor> oldestPostitions =
                topicRepository.loadOldestPosition(eventType.getTopic(), true).stream().collect(
                        Collectors.toMap(TopicPosition::getPartition, Cursor::fromTopicPosition));

        return newestPostitions.entrySet().stream()
                .map(e -> new TopicPartition(
                        eventType.getName(),
                        e.getValue().getPartition(),
                        oldestPostitions.get(e.getKey()).getOffset(),
