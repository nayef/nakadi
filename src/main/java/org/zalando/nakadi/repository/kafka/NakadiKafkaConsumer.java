...
public class NakadiKafkaConsumer implements EventConsumer {
    ...
    private void pollFromKafka() {
        ...
        eventQueue = StreamSupport
                ...
                .map(record -> {
                    final KafkaCursor nextCursor = new KafkaCursor(record.topic(), record.partition(), record.offset())
                            .addOffset(1L);
                    return new ConsumedEvent(record.value(), nextCursor.toNakadiPosition());
                })
