...
public class KafkaTopicRepositoryTest {
    ...
    public void validateValidCursors() throws NakadiException, InvalidCursorException {
        ...
        for (final Cursor cursor : MY_TOPIC_VALID_CURSORS) {
            kafkaTopicRepository.createEventConsumer(asTopicPosition(MY_TOPIC, asList(cursor)));
        }
        // validate all valid cursors
        kafkaTopicRepository.createEventConsumer(asTopicPosition(MY_TOPIC, MY_TOPIC_VALID_CURSORS));

        // validate each individual valid cursor
        for (final Cursor cursor : ANOTHER_TOPIC_VALID_CURSORS) {
            kafkaTopicRepository.createEventConsumer(asTopicPosition(ANOTHER_TOPIC, asList(cursor)));
        }
        // validate all valid cursors
        kafkaTopicRepository.createEventConsumer(asTopicPosition(ANOTHER_TOPIC, ANOTHER_TOPIC_VALID_CURSORS));
    }
    ...
    public void invalidateInvalidCursors() throws NakadiException {
        ...
        try {
            kafkaTopicRepository.createEventConsumer(asTopicPosition(MY_TOPIC, asList(outOfBoundOffset)));
        ...
        try {
            kafkaTopicRepository.createEventConsumer(asTopicPosition(MY_TOPIC, asList(nonExistingPartition)));
        } catch (final InvalidCursorException e) {
        ...
        try {
            kafkaTopicRepository.createEventConsumer(asTopicPosition(MY_TOPIC, asList(wrongOffset)));
        } catch (final InvalidCursorException e) {
