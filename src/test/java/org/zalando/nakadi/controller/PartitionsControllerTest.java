...
public class PartitionsControllerTest {
    ...
    private static final TopicPosition TEST_TOPIC_POSITION_0_NEWEST =
            new TopicPosition(EVENT_TYPE.getTopic(), "0", "67");
    private static final TopicPosition TEST_TOPIC_POSITION_1_NEWEST =
            new TopicPosition(EVENT_TYPE.getTopic(), "1", "98");
    private static final List<TopicPosition> TEST_TOPIC_POSITIONS_NEWEST = ImmutableList.of(
            TEST_TOPIC_POSITION_0_NEWEST,
            TEST_TOPIC_POSITION_1_NEWEST);

    private static final TopicPosition TEST_TOPIC_POSITION_0_OLDEST =
            new TopicPosition(EVENT_TYPE.getTopic(), "0", "12");
    private static final TopicPosition TEST_TOPIC_POSITION_1_OLDEST =
            new TopicPosition(EVENT_TYPE.getTopic(), "1", "43");
    private static final List<TopicPosition> TEST_TOPIC_POSITIONS_OLDEST = ImmutableList.of(
            TEST_TOPIC_POSITION_0_OLDEST,
            TEST_TOPIC_POSITION_1_OLDEST);
    ...
    public void whenListPartitionsThenOk() throws Exception {
        ...
        when(topicRepositoryMock.loadNewestPosition(EVENT_TYPE.getTopic())).thenReturn(TEST_TOPIC_POSITIONS_NEWEST);
        when(topicRepositoryMock.loadOldestPosition(eq(EVENT_TYPE.getTopic()), anyBoolean())).thenReturn(
                TEST_TOPIC_POSITIONS_OLDEST);
    ...
    public void whenGetPartitionThenOk() throws Exception {
        when(eventTypeRepositoryMock.findByName(TEST_EVENT_TYPE)).thenReturn(EVENT_TYPE);
        when(topicRepositoryMock.topicExists(eq(EVENT_TYPE.getTopic()))).thenReturn(true);
        when(topicRepositoryMock.loadNewestPosition(eq(EVENT_TYPE.getTopic()))).thenReturn(TEST_TOPIC_POSITIONS_NEWEST);
        when(topicRepositoryMock.loadOldestPosition(eq(EVENT_TYPE.getTopic()), anyBoolean()))
                .thenReturn(TEST_TOPIC_POSITIONS_OLDEST);
